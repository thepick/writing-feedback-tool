...
function parseDetailedAssessment(step3Text, originalText, optGenreInfo) {
    var result = {
        categories: {},
        strength: "",
        growGoal: "",
    ...
    result.notebookGuideCandidates = parseNotebookGuideCandidatesFromStep3Text(step3Text, originalText, optGenreInfo);
    return result;
}
...
function parseFirstJsonObject(text) {
    ...
    if (start !== -1 && end > start) {
        try { return JSON.parse(value.slice(start, end + 1)); } catch (e) { }
    }
    return null;
}
...
function notebookGuideExampleHasTextConnection(originalQuote, nextTimeExample) {
    ...
    var matches = 0;
    // Short quotes must be able to change a verb or replace a vague word.
    var needed = Math.min(3, Math.max(1, Math.ceil(originalWords.length / 2)));
...
function buildNotebookGuidePriorities(data) {
    return buildNotebookGuidePrioritiesFromRawInput(data);
}
...
// Preserve saved examples. Build missing guides from available assessment evidence.
function ensureNotebookGuide(data, originalText) {
    data = data || {};
    if (data.notebookGuide && Number(data.notebookGuide.version || data.notebookGuideVersion) >= NOTEBOOK_GUIDE_VERSION) return data.notebookGuide;
    if (data.sampleStatus && data.sampleStatus.status !== "scorable") {
        var goal = getGoalPlan(data);
        data.notebookGuide = {
            version: NOTEBOOK_GUIDE_VERSION,
            writingLabel: getNotebookGuideWritingLabel(data.writingGenreInfo || data.writingGenre),
            focusItems: [goal.nextTime],
            examples: [],
            quickChecks: goal.checklist
        };
        data.notebookGuideVersion = NOTEBOOK_GUIDE_VERSION;
        return data.notebookGuide;
    }
    var priorities = buildNotebookGuidePriorities(data);
    var genreInfo = data.writingGenreInfo || data.writingGenre || detectWritingGenreInfo(originalText || "");
    var source = Object.assign({}, data, { correctedText: data.correctedStory || data.correctedText || originalText });
    var candidates = data.detailed && data.detailed.notebookGuideCandidates || [];
    data.notebookGuide = assembleNotebookGuideFromCandidates(candidates, originalText, genreInfo, priorities, source);
    data.notebookGuideVersion = NOTEBOOK_GUIDE_VERSION;
    return data.notebookGuide;
}

function getNotebookGuideFromPortfolioSession(session) {
    var data = buildNotebookReconstructionDataFromSession(session);
    data.notebookGuide = session.notebookGuide;
    data.notebookGuideVersion = session.notebookGuideVersion;
    return ensureNotebookGuide(data, session.originalText || "");
}
...
function fillNotebookSummary() {
    ...
    setWftSanitizedInnerHtml("notebookPage2Content", renderNotebookGuideHtml(ensureNotebookGuide(latestAnalysisData, text), getNotebookPrintGuideRenderOptions()));
    return true;
}
...
function getNotebookPrintCss() {
    return [
        ...
        ".screen-controls { text-align: center; padding: 14.143px; font-family: 'DM Sans', 'Helvetica Neue', sans-serif; font-size: 15.557px; color: #555; }",
        ".btn-print { display: inline-block; background: #222; color: #fff; border: none; padding: 8.486px 28.286px; border-radius: 5.657px; font-size: 15.557px; font-family: 'DM Sans', 'Helvetica Neue', sans-serif; font-weight: 600; cursor: pointer; margin-top: 8.486px; }",
        ".btn-print:hover { background: #000; }",
        ".page { width: 210mm; min-height: 297mm; background: #ffffff; margin: 11.314mm auto; padding: 11.314mm 12.729mm 12.729mm; box-shadow: 0 2.829px 16.971px rgba(0,0,0,0.18); page-break-after: always; }",
        ".page:last-child { page-break-after: auto; }",
        ".page-header { display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 2.829px solid #333333; padding-bottom: 5.657px; margin-bottom: 7.071px; }",
        ".page-label { font-size: 12.729px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.09em; color: #444444; margin-bottom: 2.829px; }",
        ".page-title { font-family: 'DM Serif Display', Georgia, serif; font-size: 26.164px; font-weight: 400; color: #111111; line-height: 1.1; }",
        ".overall-score { text-align: right; flex-shrink: 0; margin-left: 11.314px; }",
        ".overall-label { font-size: 12.021px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.09em; color: #444444; margin-bottom: 1.414px; }",
        ".overall-value { font-family: 'DM Serif Display', Georgia, serif; font-size: 37.479px; font-weight: 400; color: #111111; line-height: 1; }",
        ".meta-row { display: flex; gap: 19.8px; font-size: 14.567px; color: #444444; padding: 4.243px 0 5.657px; border-bottom: 1.414px solid #cccccc; margin-bottom: 7.071px; }",
        ".meta-row strong { color: #111111; font-weight: 600; }",
        ".top-boxes { display: grid; grid-template-columns: 1fr 1fr; gap: 7.071px; margin-bottom: 7.071px; }",
        ".info-box { background: #f5f5f5; border: 1.414px solid #cccccc; border-radius: 5.657px; padding: 5.657px 7.071px; }",
        ".box-label { font-size: 12.729px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: #444444; margin-bottom: 2.829px; }",
        ".info-box p { font-size: 14.001px; color: #111111; line-height: 1.45; }",
        ".teacher-comment { background: #f5f5f5; border: 1.414px solid #cccccc; border-left: 4.243px solid #333333; border-radius: 5.657px; padding: 5.657px 7.071px; margin-bottom: 8.486px; }",
        ".teacher-comment p { font-size: 14.001px; color: #111111; line-height: 1.5; }",
        ".section-title { font-size: 12.729px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.09em; color: #444444; border-bottom: 1.414px solid #888888; padding-bottom: 2.829px; margin-bottom: 7.071px; }",
        ".assessment-grid { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 5.657px 8.486px; align-items: start; }",
        ".category { border: 1.414px solid #cccccc; border-radius: 4.243px; padding: 5.657px 7.071px; background: #ffffff; width: 100%; min-width: 0; }",
        ".category-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 2.829px; width: 100%; min-width: 0; }",
        ".category-name { font-weight: 700; font-size: 14.567px; color: #111111; letter-spacing: 0.01em; min-width: 0; }",
        ".score-badge { font-size: 13.153px; font-weight: 700; background: #ececec; border: 1.414px solid #888888; border-radius: 140.014px; padding: 0px 7.071px; white-space: nowrap; color: #111111; }",
        ".score-bar-track { display: block; width: 100%; min-width: 0; height: 4.95px; min-height: 4.95px; background: #cccccc; border-radius: 140.014px; margin-bottom: 5.657px; overflow: hidden; }",
        ".score-bar-fill { display: block; height: 100%; min-height: 100%; border-radius: 140.014px; background: #333333; }",
        ".evidence-block { font-size: 13.153px; color: #111111; line-height: 1.45; margin-bottom: 4.243px; }",
        ".evidence-block strong, .tip-block strong { font-weight: 700; color: #111111; }",
        ".tip-block { font-size: 13.153px; color: #444444; line-height: 1.45; padding-top: 4.243px; border-top: 1.414px dashed #cccccc; }",
        ".page2-header { display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 2.829px solid #333333; padding-bottom: 5.657px; margin-bottom: 11.314mm; }",
        ".page2-meta { font-size: 11.314px; color: #444444; margin-bottom: 2.829px; }",
        ".page2-title { font-family: 'DM Serif Display', Georgia, serif; font-size: 19.8px; font-weight: 400; }",
        ".page2-date { font-size: 12.021px; color: #444444; text-align: right; }",
        ".corrected-writing { border: 1.414px solid #cccccc; border-radius: 5.657px; padding: 8.486mm 9.9mm; }",
        ".corrected-writing .section-title { margin-bottom: 4.243mm; }",
        ".corrected-writing p, .corrected-writing .notebook-corrected-text { font-size: 14.143px; color: #111111; line-height: 1.45; margin-bottom: 4.243mm; white-space: normal; }",
        ".corrected-writing p:last-child { margin-bottom: 0; }",
        ".corrected-writing b, .corrected-writing .corrected-highlight { font-weight: 700; text-decoration: underline; text-decoration-thickness: 1.414px; background: transparent; color: #111111; padding: 0; border-radius: 0; }",
        ".corrected-writing .story-title-line { display: block; font-weight: 700; margin-bottom: 4.243mm; color: #111111; }",
        ".next-time-guide { border: 1.414px solid #cccccc; border-top: 4.243px solid #3b2f45; border-radius: 5.657px; padding: 3.536mm 4.243mm; font-size: 13.011px; line-height: 1.28; color: #111111; }",
        ".next-time-guide .section-title { color: #3b2f45; border-bottom-color: #d6d3d1; margin-bottom: 2.546mm; }",
        ".guide-intro { font-size: 12.729px; line-height: 1.28; color: #374151; margin-bottom: 2.829mm; }",
        ".next-time-focus, .quick-check { font-size: 12.163px; color: #374151; background: #fafaf9; border-left: 2.829px solid #d97706; padding: 2.121mm 2.829mm; margin-bottom: 2.829mm; line-height: 1.25; }",
        ".next-time-focus-title, .quick-check-title, .example-heading { font-size: 11.031px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #92400e; margin-bottom: 1.131mm; }",
        ".next-time-focus ul, .quick-check ul { margin: 0; padding-left: 4.95mm; }",
        ".next-time-focus li, .quick-check li { margin: 0 0 0.566mm; }",
        ".guide-example { border: 1.414px solid #e5e7eb; border-radius: 4.243px; padding: 2.121mm 2.546mm; margin-bottom: 2.404mm; break-inside: avoid; page-break-inside: avoid; }",
        ".example-skill { font-size: 12.729px; font-weight: 700; color: #111827; margin-bottom: 1.414mm; }",
        ".example-label { font-size: 11.031px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280; margin-top: 0.99mm; }",
        ".example-quote, .example-after, .example-why, .guide-no-examples, .quick-check-intro { font-size: 12.304px; line-height: 1.26; color: #111111; }",
        ".example-quote, .example-after { background: #ffffff; border-left: 2.829px solid #d6d3d1; padding: 0.849mm 1.697mm; margin-top: 0.566mm; }",
        ".example-why { margin-top: 0.566mm; }",
        ".auto-fit-page { --fit-scale: 1; }",
        ".auto-fit-page .page-header { padding-bottom: calc(5.657px * var(--fit-scale)); margin-bottom: calc(7.071px * var(--fit-scale)); }",
        ".auto-fit-page .page-label { font-size: calc(12.729px * var(--fit-scale)); margin-bottom: calc(2.829px * var(--fit-scale)); }",
        ".auto-fit-page .page-title { font-size: calc(26.164px * var(--fit-scale)); }",
        ".auto-fit-page .overall-label { font-size: calc(12.021px * var(--fit-scale)); }",
        ".auto-fit-page .overall-value { font-size: calc(37.479px * var(--fit-scale)); }",
        ".auto-fit-page .meta-row { gap: calc(19.8px * var(--fit-scale)); font-size: calc(14.567px * var(--fit-scale)); padding-top: calc(4.243px * var(--fit-scale)); padding-bottom: calc(5.657px * var(--fit-scale)); margin-bottom: calc(7.071px * var(--fit-scale)); }",
        ".auto-fit-page .top-boxes { gap: calc(7.071px * var(--fit-scale)); margin-bottom: calc(7.071px * var(--fit-scale)); }",
        ".auto-fit-page .info-box, .auto-fit-page .teacher-comment { padding: calc(5.657px * var(--fit-scale)) calc(7.071px * var(--fit-scale)); }",
        ".auto-fit-page .teacher-comment { margin-bottom: calc(8.486px * var(--fit-scale)); }",
        ".auto-fit-page .box-label, .auto-fit-page .section-title { font-size: calc(12.729px * var(--fit-scale)); }",
        ".auto-fit-page .box-label { margin-bottom: calc(2.829px * var(--fit-scale)); }",
        ".auto-fit-page .info-box p, .auto-fit-page .teacher-comment p { font-size: calc(14.001px * var(--fit-scale)); line-height: 1.32; }",
        ".auto-fit-page .section-title { padding-bottom: calc(2.829px * var(--fit-scale)); margin-bottom: calc(7.071px * var(--fit-scale)); }",
        ".auto-fit-page .assessment-grid { gap: calc(5.657px * var(--fit-scale)) calc(8.486px * var(--fit-scale)); }",
        ".auto-fit-page .category { padding: calc(5.657px * var(--fit-scale)) calc(7.071px * var(--fit-scale)); }",
        ".auto-fit-page .category-header { margin-bottom: calc(2.829px * var(--fit-scale)); }",
        ".auto-fit-page .category-name { font-size: calc(14.567px * var(--fit-scale)); }",
        ".auto-fit-page .score-badge { font-size: calc(13.153px * var(--fit-scale)); padding-left: calc(7.071px * var(--fit-scale)); padding-right: calc(7.071px * var(--fit-scale)); }",
        ".auto-fit-page .score-bar-track { width: 100%; min-width: 0; height: 4.95px; min-height: 4.95px; margin-bottom: calc(5.657px * var(--fit-scale)); }",
        ".auto-fit-page .evidence-block, .auto-fit-page .tip-block { font-size: calc(13.153px * var(--fit-scale)); line-height: 1.32; }",
        ".auto-fit-page .evidence-block { margin-bottom: calc(4.243px * var(--fit-scale)); }",
        ".auto-fit-page .tip-block { padding-top: calc(4.243px * var(--fit-scale)); }",
        ".auto-fit-page .page2-header { padding-bottom: calc(5.657px * var(--fit-scale)); margin-bottom: calc(4.243mm * var(--fit-scale)); }",
        ".auto-fit-page .page2-meta { font-size: calc(11.314px * var(--fit-scale)); margin-bottom: calc(2.829px * var(--fit-scale)); }",
        ".auto-fit-page .page2-title { font-size: calc(19.8px * var(--fit-scale)); }",
        ".auto-fit-page .page2-date { font-size: calc(12.021px * var(--fit-scale)); }",
        ".auto-fit-page .next-time-guide { padding: calc(3.536mm * var(--fit-scale)) calc(4.243mm * var(--fit-scale)); font-size: calc(13.011px * var(--fit-scale)); }",
        ".auto-fit-page .next-time-guide .section-title { margin-bottom: calc(2.546mm * var(--fit-scale)); }",
        ".auto-fit-page .guide-intro { font-size: calc(12.729px * var(--fit-scale)); margin-bottom: calc(2.829mm * var(--fit-scale)); }",
        ".auto-fit-page .next-time-focus, .auto-fit-page .quick-check { font-size: calc(12.163px * var(--fit-scale)); padding: calc(2.121mm * var(--fit-scale)) calc(2.829mm * var(--fit-scale)); margin-bottom: calc(2.829mm * var(--fit-scale)); }",
        ".auto-fit-page .next-time-focus-title, .auto-fit-page .quick-check-title, .auto-fit-page .example-heading { font-size: calc(11.031px * var(--fit-scale)); margin-bottom: calc(1.131mm * var(--fit-scale)); }",
        ".auto-fit-page .next-time-focus ul, .auto-fit-page .quick-check ul { padding-left: calc(4.95mm * var(--fit-scale)); }",
        ".auto-fit-page .next-time-focus li, .auto-fit-page .quick-check li { margin-bottom: calc(0.566mm * var(--fit-scale)); }",
        ".auto-fit-page .guide-example { padding: calc(2.121mm * var(--fit-scale)) calc(2.546mm * var(--fit-scale)); margin-bottom: calc(2.404mm * var(--fit-scale)); }",
        ".auto-fit-page .example-skill { font-size: calc(12.729px * var(--fit-scale)); margin-bottom: calc(1.414mm * var(--fit-scale)); }",
        ".auto-fit-page .example-label { font-size: calc(11.031px * var(--fit-scale)); margin-top: calc(0.99mm * var(--fit-scale)); }",
        ...
        // Color has the same meaning on both pages: teal for strengths/revisions,
        // amber for next steps, and slate for neutral assessment information.
        ".page { --ink: #293b43; --muted: #59666c; --line: #d7dedf; --teal: #28665e; --teal-soft: #edf5f2; --amber: #875f2d; --amber-soft: #faf4e9; }",
        ".page-header, .page2-header { border-bottom-color: var(--ink); }",
        ".page-title, .page2-title, .overall-value, .category-name, .section-title, .example-skill { color: var(--ink); }",
        ".page-label, .page2-meta, .overall-label, .meta-row, .page2-date, .example-label { color: var(--muted); }",
        ".section-title { border-bottom-color: var(--line); }",
        ".info-box { background: var(--amber-soft); border-color: #e6d8c3; }",
        ".info-box .box-label { color: var(--amber); }",
        ".info-box:first-child { background: var(--teal-soft); border-color: #cbded7; }",
        ".info-box:first-child .box-label { color: var(--teal); }",
        ".teacher-comment { background: #f5f6f6; border-color: var(--line); border-left-color: #839398; }",
        ".teacher-comment .box-label { color: var(--ink); }",
        ".category, .guide-example { border-color: var(--line); }",
        ".score-badge { color: var(--ink); background: #f0f3f3; border-color: var(--line); }",
        ".score-bar-track { background: #e7eded; }",
        ".score-bar-fill { background: #71888c; }",
        ".next-time-guide { border-color: var(--line); border-top-color: var(--ink); }",
        ".next-time-guide .section-title { color: var(--ink); border-bottom-color: var(--line); }",
        ".next-time-focus, .quick-check { background: var(--amber-soft); border-left-color: #b38b56; }",
        ".next-time-focus-title, .quick-check-title { color: var(--amber); }",
        ".example-heading { color: var(--muted); }",
        ".example-quote { background: #f5f6f6; border-left-color: #c3cdcf; }",
        ".example-after { background: var(--teal-soft); border-left-color: var(--teal); }",
...
function buildNotebookPrintDocument(printContentHtml, studentName, writingTitle) {
    ...
    return '<!DOCTYPE html>' +
        ...
        '<div class="screen-controls">Writing Notebook Summary - A4 (2 pages; double-sided: flip on long edge) &nbsp;|&nbsp; <button class="btn-print" onclick="window.print()">Print / Save as PDF</button></div>' +
...
function buildNotebookPrintHtmlFromPortfolioSession(studentName, session) {
    ...
    return ''
        ...
        + '<div class="next-time-guide">' + renderNotebookGuideHtml(getNotebookGuideFromPortfolioSession(session), getNotebookPrintGuideRenderOptions()) + '</div></div>';
}
...
function refreshNotebookPage2Guide(printContentHtml, studentName, session) {
    var template = document.createElement("template");
    template.innerHTML = sanitizeWftHtmlFragment(String(printContentHtml || ""));
    var pages = template.content.querySelectorAll(".page");
    var page2 = pages[1];
    var content = page2 && page2.querySelector(".next-time-guide, .corrected-writing");
    if (!content) return buildNotebookPrintHtmlFromPortfolioSession(studentName, session);
    page2.classList.add("auto-fit-page");
    // Keep a historical guide when no structured guide was saved with that snapshot.
    if (content.classList.contains("next-time-guide") && !session.notebookGuide) return template.innerHTML;
    content.className = "next-time-guide";
    content.id = "notebookPage2Content";
    content.innerHTML = renderNotebookGuideHtml(getNotebookGuideFromPortfolioSession(session), getNotebookPrintGuideRenderOptions());
...
function printPortfolioNotebookSummary(studentName, sessionId) {
    ...
    printContentHtml = refreshNotebookPage2Guide(printContentHtml, studentName, session);
    var printWindow = window.open("", "_blank", "width=1200,height=900");
    if (!printWindow) {
...
async function analyzeWriting() {
    ...
    try {
        ...
        if (sampleStatus.status !== "scorable") {
            ...
            ensureNotebookGuide(latestAnalysisData, text);
        ...
        var notebookGuidePriorities = buildNotebookGuidePriorities({
            categoryScores: Object.fromEntries(Object.keys(parsed1.quickRubric).map(function(key) {
                return [key, parsed1.quickRubric[key].score];
            })),
            flowData: flowData
        });
        ...
        var detailed = parseDetailedAssessment(step3, text, writingGenreInfo);
